...
ASSETS: Dict[str, Dict[str, str]] = {
    ...
    "EU": {
        # ETF liquides pour limiter les trous de données
        "equity": "FEZ",  # Euro Stoxx 50 ETF (US listing, données stables)
        "bond": "BNDX",  # Obligations internationales couvertes USD
        "gold": "GLD",
        "oil": "BZ=F",  # Brent
    },
}
...
def load_zone_data(zone: str, period: str = DEFAULT_PERIOD) -> pd.DataFrame:
    ...
    if df.empty:
        raise ValueError(f"Pas de données exploitables pour la zone {zone}. Vérifie les tickers ou la période.")

    # Vérification explicite des noms attendus pour éviter les KeyError plus loin.
    missing_cols = set(assets.keys()) - set(df.columns)
    if missing_cols:
        raise ValueError(f"Colonnes manquantes pour {zone}: {missing_cols}. Téléchargement incomplet.")
